...
import { isYouTubeUrl, transcribeYouTube } from '@/services/youtube/client';
...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/health').then((response) => response.json()).then((data) => setConfigured(Boolean(data.transcriptionConfigured))).catch(() => setConfigured(null));
  }, []);
  ...
  const startTranscription = async () => {
    ...
    setError(null); setProgress(7); setPhase('processing');
    const timer = window.setInterval(() => setProgress((value) => Math.min(88, value + (value < 40 ? 7 : value < 70 ? 4 : 1.5))), 700);
    try {
      const transcript = source === 'youtube' ? await transcribeYouTube(youtubeUrl) : await transcribeFile(file!, source);
      window.clearInterval(timer); setProgress(100); setResult(transcript);
      window.setTimeout(() => { setPhase('result'); window.scrollTo({ top: 0, behavior: 'smooth' }); }, 480);
    } catch (caught) {
      window.clearInterval(timer); setError(caught instanceof Error ? caught.message : 'The mission failed unexpectedly. Please try again.');
  ...
  const clear = () => {
    if (mediaUrl) URL.revokeObjectURL(mediaUrl);
    setPhase('choose'); setSource(null); setFile(null); setDetails(null); setYoutubeUrl(''); setMediaUrl(undefined); setResult(null); setError(null); setProgress(0);
  ...
  return (
    <main className={`app-shell ${phase === 'result' ? 'result-mode' : ''}`}>
      ...
      <nav className="topbar" aria-label="Primary navigation">
        ...
        <div className={`status-pill ${configured === false ? 'needs-key' : ''}`}><span /> {configured === false ? 'API KEY NEEDED' : 'SYSTEM READY'}</div>
      </nav>
      ...
      {phase === 'ready' && source && <section className="mission-panel" ref={missionRef}>
        ...
        {source === 'youtube' && <div className="youtube-entry"><label htmlFor="youtube-url">YOUTUBE MISSION URL</label><div><span>↗</span><input id="youtube-url" value={youtubeUrl} onChange={(event) => { setYoutubeUrl(event.target.value); setError(null); }} placeholder="https://youtube.com/watch?v=..." inputMode="url" autoComplete="url" /></div><p>We retrieve accessible subtitles first. If YouTube blocks access, you will be asked to upload the media instead.</p></div>}
        {configured === false && source !== 'youtube' && <div className="setup-notice"><span>!</span><p><strong>ONE-TIME SETUP NEEDED</strong>Add <code>OPENAI_API_KEY</code> to the app environment before live transcription.</p></div>}
      ...
      {phase === 'processing' && source && <ProcessingScreen progress={progress} source={source} />}
